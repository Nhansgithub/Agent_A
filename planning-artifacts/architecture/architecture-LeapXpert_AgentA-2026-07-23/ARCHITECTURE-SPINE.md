...
### AD-26 — Inline-comment feedback enters through the existing review loop [AMENDMENT 2026-07-26]

- **Binds:** the webhook router, the tenant router, the Confluence adapter, the Feedback interpreter, the orchestrator.
- **Prevents:** a reviewer's Confluence inline comment on the draft being lost (there was no ingress for it), and a second, forked feedback path that would duplicate — and drift from — the conversation-aware review loop.
- **Rule:** a `ConfluenceCommentEvent` (a *Page commented* Automation rule, `webhookEvent: page_commented` — a marker chosen not to collide with Jira's `comment_created`) is deduped on the comment id (globally unique; no version marker) and routed by space key / single-tenant fallback (a comment payload rarely carries a container folder). Dispatch acts **only** when the comment's page is a run's `userdoc_page_id` and the run is under review; every other comment (footer, other pages, off-review) is read and ignored, so the space-wide trigger is safe. Only the **adapter** reads the comment (AD-1): `get_inline_comment` uses the **v1** `content/{id}` endpoint as primary (the v2 `inline-comments/{id}` endpoint 404s intermittently, and v1 also reports `location` to tell an inline comment from a footer one), falling back to v2, parsing either shape; the highlighted passage (`inlineProperties.originalSelection` / `properties.inlineOriginalSelection`) is the "section". `apply_inline_comment` restates the note into `Section / Issue / Suggested change` — the Feedback interpreter **proposes a fix when the reviewer gave none** — posts it on the Review ticket **@-mentioning the exact commenter** (not the config PM), and parks at `AWAITING_STRUCTURE_CONFIRM` with the restatement as `pending_feedback`: the identical state a plain-language PM comment reaches, so the reply is driven by the **existing** `apply_pm_comment` conversation loop (AD-16 routing unchanged), not a parallel one. The commenter is recorded in a new `active_reviewer_account_id` column (idempotent additive migration) and the review-loop @-mention helpers address them for the whole confirmation sub-conversation, clearing it once the feedback is applied or abandoned.
